...
|           |                                                              | **FN027** | Guest Guard      | ログイン済みユーザーが /login, /register にアクセスした際、ホームへリダイレクトしセッション不整合を防止。 |
| **US003** | ログインユーザーは，システムからログアウトすることができる。 | **FN008** | ログアウト機能   | トークン破棄、Cookie削除、および React Query キャッシュの完全クリアを行い、前ユーザーのデータ残存を防止。 |
...
|           |                                                            | **FN028** | キャッシュ分離 | クエリキーに user.id を含め、ログイン状態の変化やユーザー切り替え時に古いキャッシュが表示されないよう制御。 |
...
|           |                                                          | **FN023** | 投稿一覧タブ     | 「ツイート」と「いいね」を切り替え。ただし「いいね」タブは自身のプロフィール時のみ表示（プライバシー保護）。 |
...
|           |                                                            | **FN029** | 未ログインガード   | 未ログイン時の「いいね」「返信」時に「ログインが必要です。」トーストを表示し、返信フォーム展開を阻止。 |
