...
完了していること: 認証（登録）機能のテスト完遂。連打防止・サーバーダウン・通信断等の過酷なエッジケースを克服。これに伴い、Supabase SDK のエラーを完全に解体し日本語化する「無敵のエラーマッピング」を確立。テスト基盤および OS 非依存の実行環境も整備済み。
未完了・次のタスク: ログイン、タイムライン、いいね等の残りのテスト実装。
