...
完了していること: 実機 DB を用いた E2E テスト基盤（物理バックアップ、IPv4 直指定、タイムアウト延長）の確立を完遂。Playwright による最初の実機証明（認証フロー: ID 1-5, 1-17）に成功。104 件の Vitest テストと 2 件の E2E テストがすべて Green、Lint 合格。
未完了・次のタスク: 投稿・プロフィール更新連動 (ID 2-3, 5-1) および認可ガード (ID 5-4) の E2E テストによる実機証明。
