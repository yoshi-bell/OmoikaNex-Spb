...
完了していること: タイムラインのタブ切り替え（おすすめ/フォロー中）の実装が完了。Phase 5 の主要な UX 改善（相対日時、投稿時の自動スクロール、スケルトンローディングへの刷新）および ESLint 警告の解消を完遂。
未完了・次のタスク: プロフィール編集機能（ユーザー名・自己紹介・プロフィール画像）の実装。
