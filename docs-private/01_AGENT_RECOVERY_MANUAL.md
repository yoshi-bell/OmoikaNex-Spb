...
完了していること: 認証、投稿、タイムライン、コメント、削除、いいね、およびフォロー機能（プロフィール表示制御含む）のテスト完遂。楽観的更新の物理的証明、スレッド内不整合の解消、および他者プロフィールのプライバシー保護を検証済み。全ユニットテスト 93 件が Green。
未完了・次のタスク: フォロー中タイムラインの厳密な検証 (ID 4-6)、自己フォロー防止 (ID 4-9) の Repository テスト、およびプロフィール更新（セクション 5）のテスト実装。
