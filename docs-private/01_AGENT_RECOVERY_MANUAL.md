...
完了していること: タイムラインのタブ切り替え（おすすめ/フォロー中）の実装が完了。Phase 5 の主要な UX 改善（相対日時、投稿時の自動スクロール、スケルトンローディングへの刷新）および ESLint 警告の解消を完遂。Phase 6 基盤として `avatar_url` 追加、Storage 設定、および `db reset` による環境再構築を完了。
未完了・次のタスク: プロフィール編集機能（ユーザー名・自己紹介・プロフィール画像）の UI および API の実装。
