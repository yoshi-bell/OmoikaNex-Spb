...
完了していること: 認証、エラーマッピング、投稿 UI、およびタイムライン取得（無限スクロール）のテスト完遂。全テストコードからの `any` 排除（型安全リファクタリング）を完了。QA 指摘の全攻撃シナリオ（脆弱性・UX 欠陥）を攻略済み。全ユニットテスト 57 件が Green。
未完了・次のタスク: ツイート削除、コメント、いいね、フォロー、プロフィール更新等の残りのテスト実装。
