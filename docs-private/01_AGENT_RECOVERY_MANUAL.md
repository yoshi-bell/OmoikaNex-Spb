...
完了していること: 認証、投稿、タイムライン、コメント、削除、および「いいね」機能（楽観的更新）の全方位テストを完遂。UI レベルでの瞬時更新と一括ロールバックを物理的に証明済み。React Query の Observer 通知最適化（setQueryDataへの移行）を完了。全ユニットテスト 88 件が Green。
未完了・次のタスク: フォロー機能（エンゲージメント）、プロフィール更新等の残りのテスト実装。
