...
完了していること: 認証、エラーマッピング、投稿・タイムライン、コメント（返信）、およびツイート削除機能（ID 2-10）のテスト完遂。Supabase のサイレントな失敗（0行削除）を克服し、try-catch 防壁の強度も証明済み。全ユニットテスト 66 件が Green。
未完了・次のタスク: いいね・フォロー機能（エンゲージメント）、プロフィール更新等の残りのテスト実装。
