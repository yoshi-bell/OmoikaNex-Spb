...
完了していること: 認証、エラーマッピング、投稿 UI、タイムライン、およびツイートカード（削除・いいね制御）のテスト完遂。全テストコードの `any` 排除を完了。QA 指摘の全攻撃シナリオ（スパム、レースコンディション、UX 誤爆）を攻略済み。全ユニットテスト 62 件が Green。
未完了・次のタスク: ツイート詳細、コメント（返信）一覧、フォロー機能、プロフィール更新等の残りのテスト実装。
