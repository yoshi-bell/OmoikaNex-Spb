...
完了していること: プロフィール機能の完遂に加え、更新ロジックの完全 Server Action 化（FormData 形式）を完了。Server Action のボディサイズ制限を 4MB に拡張。
未完了・次のタスク: 詳細・プロフィール画面での削除・返信の動時反映リファクタリング、メール認証フローの実装。
