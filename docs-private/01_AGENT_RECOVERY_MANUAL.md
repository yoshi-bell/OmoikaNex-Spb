...
完了していること: プロフィール全機能、サイドバーのアイデンティティ表示、および全アクションにおける「全画面同期」を実現。Middleware による Guest Guard、未ログイン時 UX の改善、およびプロフィール画面のプライバシーガード（他人のいいね非表示化）とコード共通化を完遂。
未完了・次のタスク: ログアウト時のキャッシュ不整合修正、自動テスト（Vitest, Playwright）の構築。
