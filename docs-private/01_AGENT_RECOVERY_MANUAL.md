...
完了していること: プロフィール全機能、サイドバーのアイデンティティ表示、および全アクションにおける「全画面同期」を実現。Middleware による Guest Guard、未ログイン時 UX の改善、プロフィールのプライバシー保護、およびログアウト時のキャッシュ完全同期を完遂。機能要件・テスト仕様書への反映も完了。
未完了・次のタスク: 自動テスト（Vitest, Playwright）の構築。
