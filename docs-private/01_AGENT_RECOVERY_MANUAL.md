...
完了していること: プロフィール全機能等、機能面での実装を完遂。Vitest, MSW, Playwright のテスト基盤構築、およびOS非依存（cross-env）の堅牢な実行コマンド体系の確立。さらに、`session_replication_role` を用いたプロ仕様のデータ保護・復旧フローの整備を完了。
未完了・次のタスク: `TEST_CASES.md` に基づく具体的なテストコード（単体・結合・E2E）の実装。
