...
describe("useTimeline (ID 2-8: クエリキーによるキャッシュ隔離の物理的証明)", () => {
    ...
    it("ID 2-8: [Integration] ユーザー A で実行した際、クエリキーに ユーザー A の ID が含まれていること", () => {
    ...
    it("ID 2-8: [Integration] ユーザー B に切り替わった際、クエリキーが変化し、キャッシュが物理的に隔離されること", () => {
    ...
    it("ID 2-8: [Integration] 未ログイン状態（user: null）でも、キーが崩れず安全に隔離（undefinedが含まれる）されること", () => {
        vi.mocked(useAuthUser).mockReturnValue({
            user: null,
