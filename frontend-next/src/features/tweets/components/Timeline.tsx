  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-10 space-y-4">
        ...
        <p className="text-gray-500 text-sm font-medium">タイムラインを読み込み中...</p>
      </div>
    )
  }
  ...
  if (isError || result?.error) {
    return (
      <div className="p-8 text-center bg-red-50 rounded-lg m-4">
        <p className="text-red-600 font-bold">データの取得に失敗しました</p>
        <p className="text-red-500 text-sm mt-1">
          {result?.error?.message || 'ネットワークエラーが発生しました。'}
        </p>
        <button 
          onClick={() => refetch()}
  ...
  if (tweets.length === 0) {
    return (
      <div className="p-10 text-center text-gray-500">
        ...
        <p className="text-sm mt-1">最初のツイートを投稿してみましょう！</p>
      </div>
    )
  }
